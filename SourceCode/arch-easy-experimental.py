...
class InstallerUI:
    ...
    def run_steps(self):
        ...
        # build inner script that follows the exact logic of your working snippet,
        # but uses chpasswd (Arch-compatible) and decodes base64 inside the chroot
        inner_cmds = []
        ...
        # this line decodes root:user and user:user from base64 and feeds chpasswd (exact semantics of working snippet)
        inner_cmds.append(f"printf '%s\\n' '{rp_b64}' '{up_b64}' | base64 -d | chpasswd")
        inner_cmds.append("sed -i 's/^# %wheel ALL=(ALL:ALL) ALL/%wheel ALL=(ALL:ALL) ALL/' /etc/sudoers")
...
if __name__ == "__main__":
    ...
    try:
        ...
        fallback_cli()#!/usr/bin/env python3
import curses
import subprocess
import sys
import os
