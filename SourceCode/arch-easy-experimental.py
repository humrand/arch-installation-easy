...
class InstallerUI:
    ...
    def run_steps(self):
        ...
        # === HERE: use exactly the snippet logic you provided ===
        # set local variables from TUI state (matching the snippet variables)
        hostname = state["hostname"]
        username = state["username"]
        root_pass = state["root_pass"]
        user_pass = state["user_pass"]
        swap_size = state["swap"]
        # map desktop selection to the numeric choice form used in the snippet
        if state["desktop"] == "KDE Plasma":
            desktop_choice = "1"
        elif state["desktop"] == "Cinnamon":
            desktop_choice = "2"
        ...
        append_buffer_add("Configuring system (snippet-style)...")
        try:
            with open("/mnt/etc/hostname", "w") as f:
                f.write(hostname + "\n")
        except Exception as e:
            append_buffer_add(f"ERROR writing /mnt/etc/hostname: {e}")

        # exact chroot calls from your snippet (note: passwd --stdin is not available on stock Arch)
        ...
        steps.append(("Configure system (continuation)", "true",1))
