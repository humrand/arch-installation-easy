...
class InstallerUI:
    ...
    def run_steps(self):
        ...
        # diagnostic: listar y comprobar el script antes de ejecutar
        ...
        # ejecutar script dentro del chroot de forma robusta (chmod + ejecutar)
...
class InstallerUI:
    ...
    def run_steps(self):
        ...
        try:
            os.makedirs("/mnt/root", exist_ok=True)
            with open("/mnt/root/setup_user.sh", "w") as fh:
                fh.write(setup_sh)
            os.chmod("/mnt/root/setup_user.sh", 0o755)
        except Exception as e:
            self.add_log(f"ERROR: no se pudo crear /mnt/root/setup_user.sh: {e}")
        ...
        steps.append(("Configure system", "arch-chroot /mnt /bin/bash /root/setup_user.sh",10))
        steps.append(("Cleanup setup script", "rm -f /mnt/root/setup_user.sh",1))
